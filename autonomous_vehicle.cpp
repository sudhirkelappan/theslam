...
const  char *view_rgb = NULL;

WbDeviceTag SLAMOutput;
WbDeviceTag gyro;
WbDeviceTag imu;
bool has_gyro = false;
bool has_imu = false;
...
void LogOpen()
{
   ...
   const char *csv_path = "/home/gpumachine/sudhir/slam.csv";
...
int main(int argc, char **argv) 
{
   ...
   //
   // Init ROS with an Image Transport Publisher
   //

     ros::init(argc, argv, "SKPBot");
     // ros::Publisher pub =  nh.advertise<sensor_msgs::Image>("/stlucia/camera/image", 500);


     ros::NodeHandle nh;

     image_transport::ImageTransport it = image_transport::ImageTransport (nh);
     image_transport::Publisher pub =  it.advertise("/stlucia/camera/image", 500);
  ...
  for (j = 0; j < wb_robot_get_number_of_devices(); ++j) {
    ...
    if (strcmp(name, "Sick LMS 291") == 0)
      ...
      has_gyro = true;
    else if (strcmp(name, "VehicleIMU") == 0)
      has_imu = true;
   }
  ...
  // IMU
  if (has_imu) {
    imu = wb_robot_get_device("VehicleIMU");
  ...
  if (enable_display) {
    ...
    // SLAMOutput = wb_robot_get_device("SLAMOutput");
  }
  ...
  while (wbu_driver_step() != -1) {
	...
	view_rgb = (const char *) camera_image;
      }
      if (enable_collision_avoidance)
    ...
    ImageDataSize = camera_height * camera_width * 4;  



    // string fname = string ("imgs/") + string ("Img_from_camera_") + to_string (i) + string(".jpg");
    // cv::imwrite (fname, image);

    cv::Mat MatBGRA8 (camera_height, camera_width,  CV_8UC4, (void *)view_rgb, camera_width*4);

    if ( ! MatBGRA8.empty() )
    {

	    // cv_bridge::CvImage cvImg  = cv_bridge::CvImage(std_msgs::Header(), "bgr8", image);
	    // sensor_msgs::ImagePtr msg = cvImg.toImageMsg();


	    //
	    // We receive a bgra8 image from webots
	    //
	    cout << "Before Crash -1" << endl;


	    // cv_bridge::CvImage   im; im.header = std_msgs::Header(); im.encoding =  "bgra8"; im.image =  MatBGRA8 ;
	    // cv_bridge::CvImageConstPtr cvImgPtr  = (cv_bridge::CvImageConstPtr )& im ;


	    //
	    // Convert to BGR8 for ratslam
	    //
	    cout << "Before Crash -2" << endl;
	    cv::Mat MatBGR8;
	    cv::cvtColor(MatBGRA8, MatBGR8, cv::COLOR_BGRA2BGR);



	    //
	    // Convert to ROS ImageMsg
	    //
	    cout << "Before Crash -3" << endl;
	    cv_bridge::CvImage cvImg  = cv_bridge::CvImage(std_msgs::Header(), "bgr8", MatBGR8);
	    sensor_msgs::ImagePtr msg = cvImg.toImageMsg();


	    string fname = string ("imgs/") + string ("Img_from_camera_") + to_string (i) + string(".bmp");
	    cout << fname << endl;

	    cout << "Before Crash -4" << endl;
	    // cv::imwrite (fname, (  cv_bridge::toCvCopy (msg) )->image );
	    // cv::imwrite (fname, cvImg.image);

	    if (nh.ok()) 
	    {
		    cout << "Before Crash -5" << endl;
		    pub.publish(msg);
		    ros::spinOnce();
	    }
    }
