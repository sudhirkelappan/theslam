...
namespace ratslam
{
  ...
  void Ratslam::process()
  {
    ...
    //SKP : We are passing in total Angle so do not multiply 
    // vrot = vrot * time_diff;
    ...
    if (debug_level > 0)
    {
      ...
      LOGD("PC[ %f , %f , %f ]",network->x() , network->y() , network->th() ) ;
    }
    profile_RS_PC.stop();
    ...
    if (debug_level > 0)
    {
      ...
      LOGD("D_PC = [%f] [%f] \n", delta_pc,EXP_DELTA_PC_THRESHOLD);
    }
    ...
    if (debug_level > 0)
      LOGD("==================== PROCESS Exp size is zero Creating new Experience   ============ \n");
    ...
    else if (delta_pc > EXP_DELTA_PC_THRESHOLD || templates->get_current_vt() != prev_vt) 
    {
      ...
      if (map->get_current_id() != matched_exp_id)
      {
        if (matched_exp_id == -1)
        {
          ...
          if (debug_level > 0)
            LOGD("==================== PROCESS Exp Map Creating Experience - 1  ============ \n");
      ...
      {
        if (debug_level > 0)
          LOGD("==================== PROCESS Creating Experience   ============ \n");
